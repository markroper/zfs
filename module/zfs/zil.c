    ...
    uint64_t claim_txg)
{
	...
	/*
     * If priming is enabled, dispatch the zil replay prime function on
	 * a taskq and return, skipping the serial blocking replay function call.
	 */
	if (zfs_zil_is_priming && zr->zr_replay_prime[txtype] != NULL) {
		 ...
		 * to read into
		 */
		zrc->zr_lr = vmem_alloc(
			sizeof (lr_write_t) + ((lr_write_t *)zr->zr_lr)->lr_length,
			KM_SLEEP);
