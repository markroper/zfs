...
zfs_replay_prime_link(void *args)
{
	...
	if ((error = zfs_zget(zfsvfs, lr->lr_doid, &dzp)) != 0) {
		cmn_err(CE_WARN, "Failed to get znode for link directory "
			"%llu during replay prime: %d", lr->lr_doid, error);
		return;
	}

	if ((error = zfs_zget(zfsvfs, lr->lr_link_obj, &zp)) != 0) {
		cmn_err(CE_WARN, "Failed to get znode for link %llu "
			"during replay prime: %d", lr->lr_link_obj, error);
		zrele(dzp);
		return;
	}
